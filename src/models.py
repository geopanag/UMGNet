...
class BipartiteDraGNN(torch.nn.Module):
    def __init__(
        ...
        self.hidden_common1 = nn.Linear(
            hidden_channels + num_layers * hidden_channels, hidden_channels
        )
    ...
    def forward(self, xu: torch.tensor, xp: torch.tensor, edge_index: torch.tensor):
        out = []
...
class MLP(torch.nn.Module):
    def __init__(self, n_feat: int, hidden1: int, class_no: int = 2):
