...
class NGCF(MessagePassing):
    def __init__(self, in_dim, out_dim, dropout=0.0):
        ...
        self.W1 = torch.nn.Parameter(
            torch.nn.init.xavier_uniform_(torch.empty(in_dim, out_dim))
        )
        self.b1 = torch.nn.Parameter(
            torch.nn.init.xavier_uniform_(torch.empty(1, out_dim))
        )
    ...
    def forward(self, x, edge_index):
        return torch.nn.functional.normalize(
            self.dropout(self.propagate(edge_index, x=x)), p=2, dim=1
        )

    def message_and_aggregate(self, adj_t, x):
        side_embeddings = matmul(adj_t, x, reduce=self.aggr)
        return self.leaky_relu(
            torch.matmul(side_embeddings, self.W1)
            + self.b1
            + torch.matmul(torch.mul(side_embeddings, x), self.W2)
            + self.b2
        )
