...
def run_umgnn_al(
    ...
    cluster_assigment, cluster_budget, cluster_distance = cluster(
        xu.cpu().numpy(), sample_budget
    )
    ...
    for iter in range(0, budget_iter, step):
        ...
        if (active_policy == "egreedy") and (random.random() <= epsilon):
            # random
            new_train_indices = np.random.choice(
                test_indices, sample_budget, replace=False
            )
            test_indices = np.array(
                [i for i in test_indices if i not in new_train_indices]
            )
            train_indices += new_train_indices.tolist()
        else:
            # objective
            new_train_indices = al_lp(
                test_indices,
                degree,
                pred_uncertainty,
                cluster_distance,
        ...
        if active_policy != "random":
            mean, variance, entropy = mc_dropout(
                model, xu_, xp, edge_index_up_current, test_indices
            )
    ...
    return (
        ...
        len(train_indices),
        len(test_indices),
    )  # pd.DataFrame(result_fold).mean().values
...
def main():
    ...
    edge_index = (
        torch.tensor(edge_index_df[["user", "product"]].values)
    ...
    treatment = (
        torch.tensor(features["treatment_flg"].values).type(torch.LongTensor).to(device)
    )
    outcome_original = (
        torch.tensor(features["target"].values).type(torch.FloatTensor).to(device)
    )
    outcome_money = (
        torch.tensor(features["avg_money_after"].values)
        .type(torch.FloatTensor)
        .to(device)
    )
    outcome_change = (
        torch.tensor(features["avg_money_change"].values)
        .type(torch.FloatTensor)
        .to(device)
    )
    ...
    for task in [1, 2]:
        ...
        for budget_iter in [5, 20]:
            ...
            for run in range(0, number_of_runs):
                ...
                result_iter, len_train, len_test = run_umgnn_al(
                    outcome,
                    treatment,
                    criterion,
                    xu,
