...
class RetailHero(InMemoryDataset):
    ...
    def process(
        ...
        purchases["transaction_abs_time"] = (
            purchases["transaction_datetime"] - pd.Timestamp("1970-01-01")
        ) // pd.Timedelta("1s")
        ...
        features_purchases_before = (
            purchases_before.groupby("transaction_id")
            .agg(
                {
                    "client_id": "first",
                    "purchase_sum": "first",
        ...
        features_purchases_before = features_purchases_before.groupby("client_id").agg(
            {
                "purchase_sum": "mean",
        ...
        features_purchases_before["avg_count_before"] = features_purchases_before[
            "total_count_before"
        ] / (
            (
        ...
        labels_purchases_after = (
            purchases_after.groupby("transaction_id")
            .agg(
                {
                    "client_id": "first",
                    "purchase_sum": "first",
        ...
        labels_purchases_after = labels_purchases_after.groupby("client_id").agg(
            {
                "purchase_sum": "mean",
                "transaction_id": "count",
                "transaction_datetime": ["max", "min"],
        ...
        labels_purchases_after["avg_count_after"] = labels_purchases_after[
            ...
            (
                ...
                - labels_purchases_after["first_purchase_after"]
            ).dt.days
            + 1
        )
        ...
        purchases_before = (
            purchases_before.groupby(["client_id", "product_id", "label"])
            .sum("product_quantity")
            .reset_index()
        )
        purchases_after = (
            purchases_after.groupby(["client_id", "product_id", "label"])
            .sum("product_quantity")
            .reset_index()
        )
        ...
        purchase_processed.to_csv(
            f"{self.processed_dir}/{edge_index_file}", index=False
        )

        degrees = (
            purchase_processed[(purchase_processed["T"] == 0)]
            .groupby("user")
            .size()
            .reset_index()
        )
        ...
        ## add targets
        data = (
            df_features.join(features_purchases_before)
            .join(labels_purchases_after)
            .fillna(0)
        )
        ...
        data["user", "buys", "product"] = {
            "edge_index": torch.tensor(edge_index_df[["user", "product"]].values)
            .type(torch.LongTensor)
            .T,
            "treatment": torch.tensor(edge_index_df["T"].values).type(torch.BoolTensor),
        }

        # data['outcome_change'] = torch.tensor(features['avg_money_after'].values).type(torch.FloatTensor)
        data["user"] = {
            "x": torch.tensor(
                features[
                    [
...
class MovieLens25(InMemoryDataset):
    ...
    def process(
        ...
        outcome = np.expand_dims(
            movie_ratings[movies["movieId"].astype(int)].values, axis=0
        ).T
        ...
        data["movie", "ratedby", "user"] = {
            "edge_index": torch.tensor(edge_index_df[["movie", "user"]].values)
            .type(torch.LongTensor)
            .T,
            "treatment": torch.tensor(edge_index_df["T"].values).type(torch.BoolTensor),
        }
        ...
        for dat in range(sim_runs):
            ...
            Y = np.maximum(
                0, x[:, 0] * random_wt + x[:, 1:].dot(random_wf) + random_noise
            )
