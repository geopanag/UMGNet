...
def main():
    ...
    out_channels = 1
    num_epochs = 300
    results_mean_file_name = "../../results/results_mean.csv"
    results_sd_file_name = "../../results/results_sd.csv"
    ...
    for l2_reg in [0, 5e-4]:
        for dropout in [0.2, 0.4]:  # l2_reg = 5e-4
            for k in [10, 5]:
                for lr in [0.01, 0.001]:
                    for criterion_ in ["l1", "mse"]:
                        if criterion_ == "l1":
                        ...
                        for train_indices, test_indices in kf.split(xu):
                            ...
                            edge_index_up_current = data["user", "buys", "product"][
                                "edge_index"
                            ][
                                :,
                                (~data["user", "buys", "product"]["treatment"])
                                | (mask),
                            ]

                            edge_index_up_current[1] = (
                                edge_index_up_current[1] + xu.shape[0]
                            )

                            edge_index_up_current = torch.cat(
                                [
                                    edge_index_up_current,
                                    edge_index_up_current.flip(dims=[0]),
                                ],
                                dim=1,
                            ).to(device)
                            ...
                            uplift = learner.predict(
                                X=xu_np[train_indices],
                                treatment=treatment_np[test_indices],
                            ).squeeze()

                            uplift = learner.predict(
                                X=xu_np[test_indices],
                                treatment=treatment_np[test_indices],
                            ).squeeze()
                            ...
                            up40 = uplift_score(
                                ...
                                np.hstack(
                                    [
                                        treatment_np[train_indices],
                                        treatment_np[test_indices],
                                    ]
                                ),
                                np.hstack(
                                    [
                                        outcome_np[train_indices],
                                        outcome_np[test_indices],
                                    ]
                                ),
                                rate=0.4,
                            )
                        ...
                        v = f"{lr}_{k}_{criterion_}_{dropout}_{l2_reg}"
                        results = pd.DataFrame(
                            results,
                            columns=["up40_gnn", "up20_gnn", "up40_t", "up20_t"],
                        )  # ,'up40_x','up20_x','up40_tree','up20_tree'])

                        mean_results.append(pd.concat([results.mean(), pd.Series([v])]))
                        sd_results.append(pd.concat([results.std(), pd.Series([v])]))

                    pd.DataFrame(
                        mean_results,
                        columns=["up40_gnn", "up20_gnn", "up40_t", "up20_t", "model"],
                    ).to_csv(results_mean_file_name)

                    pd.DataFrame(
                        sd_results,
                        columns=["up40_gnn", "up20_gnn", "up40_t", "up20_t", "model"],
                    ).to_csv(results_sd_file_name)
